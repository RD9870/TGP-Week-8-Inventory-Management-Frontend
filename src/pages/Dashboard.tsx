...
function Dashboard() {
  return (
    <>
      {" "}
      {/* تغليف العناصر بـ Fragment كعنصر جذر واحد */}
      <button
      ...
      >
        <div className="h-full px-3 py-4 overflow-y-auto bg-neutral-primary-soft border-e border-default">
          <ul className="space-y-2 font-medium">
            ...
            {/* تم تحويل باقي العناصر لـ className تلقائياً بنفس الطريقة */}
            <li>
              <a
                href="#"
                className="flex items-center px-2 py-1.5 text-body rounded-base hover:bg-neutral-tertiary hover:text-fg-brand group"
        ...
        <div className="p-4 border-1 border-default border-dashed rounded-base">
          <div className="grid grid-cols-3 gap-4 mb-4">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="flex items-center justify-center h-24 rounded-base bg-neutral-secondary-soft"
              >
          ...
          {/* ... باقي الكود مع استبدال class بـ className ... */}
        </div>
      </div>
    </>
  );
